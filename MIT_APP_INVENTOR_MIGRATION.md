...
## Generate an importable `.aia` starter immediately

Use this helper to create a **real `.aia` file** you can import now:

```bash
bash scripts/create_ai2_aia_stub.sh
```

Output:
- `dist/CoreCoreSimulation.aia`

Then in MIT App Inventor:
1. `Projects > Import project (.aia) from my computer`
2. Select `dist/CoreCoreSimulation.aia`
3. Replace `Screen1` design/blocks with your simulation logic
4. Import assets from `dist/ai2_export_bundle.zip` (generated by `export_for_ai2.sh`)
